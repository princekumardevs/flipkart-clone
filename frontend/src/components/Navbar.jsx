...
function Navbar() {
  ...
  return (
    <nav className="bg-flipkart-blue fixed w-full top-0 z-50 shadow-md">
      <div className="max-w-[1248px] w-full mx-auto px-3 md:px-4 lg:px-6 py-2 md:py-0 relative">
        <div className="h-auto md:h-[56px] flex items-center gap-3 md:gap-4 lg:gap-8">
          {/* Mobile Hamburger */}
          <button
          ...
          {/* Desktop Search */}
          <form onSubmit={handleSearch} className="hidden md:block flex-1 max-w-[550px] relative">
            <input
              type="text"
              value={searchQuery}
            ...
            />
            <button type="submit" className="absolute right-3 top-1/2 -translate-y-1/2 text-flipkart-blue cursor-pointer" aria-label="Search">
          ...
          <Link to="/cart" className="md:hidden ml-auto flex items-center text-white text-[14px] font-medium">
            <div className="relative mr-1.5">
              ...
              {cartCount > 0 && (
                <span className="absolute -top-2.5 -right-2.5 bg-[#ff6161] text-white text-[10px] font-bold px-[4px] py-px rounded-full border border-flipkart-blue">
                  {cartCount}
                </span>
              )}
            </div>
        ...
        {/* Mobile Search */}
        <form onSubmit={handleSearch} className="md:hidden w-full mt-2 pb-1 relative">
          <input
            type="text"
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            placeholder="Search for products, brands and more"
            className="w-full h-[38px] pl-3.5 pr-10 rounded-sm text-[14px] bg-white text-black outline-none shadow-[0_2px_4px_0_rgba(0,0,0,.23)]"
          />
          <button type="submit" className="absolute right-3 top-1/2 -translate-y-1/2 text-flipkart-blue cursor-pointer" aria-label="Search">
            <svg width="20" height="20" viewBox="0 0 17 18" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
              <g fill="#2874F1"><path d="M11.618 9.897l4.224 4.212c.092.09.1.23.02.312l-1.464 1.46c-.08.08-.222.072-.314-.02L9.868 11.66M6.486 10.9c-2.42 0-4.38-1.955-4.38-4.367 0-2.413 1.96-4.37 4.38-4.37s4.38 1.957 4.38 4.37c0 2.412-1.96 4.368-4.38 4.368m0-10.834C2.904.066 0 2.96 0 6.533 0 10.105 2.904 13 6.486 13s6.487-2.895 6.487-6.467c0-3.572-2.905-6.467-6.487-6.467 "/></g>
        ...
        {/* Mobile Menu Overlay */}
        {mobileMenuOpen && (
          <div className="md:hidden absolute top-[108px] left-0 w-full bg-white shadow-lg text-black flex flex-col z-[100] border-t border-[#f0f0f0] max-h-[calc(100vh-108px)] overflow-y-auto">
             {user ? (
