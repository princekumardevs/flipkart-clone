...
function ProductDetail() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#f1f3f6] pt-4 px-2 pb-8">
  ...
  return (
    <div className="min-h-screen bg-[#f1f3f6] pt-4 px-[10px] pb-8">
      <div className="max-w-[1248px] mx-auto bg-white shadow-[0_1px_1px_0_rgba(0,0,0,.16)] rounded-sm flex flex-col md:flex-row min-h-[600px]">
        ...
        <div className="w-full md:w-[40%] border-r border-[#f0f0f0] flex flex-col items-center pt-8 pb-4 sticky top-[56px] self-start h-auto md:h-[calc(100vh-56px)] overflow-y-auto">
            ...
            <div className="w-[64px] flex flex-col gap-2 overflow-y-auto scrollbar-hide">
              {images.map((img, index) => (
                <div 
                  ...
                  className={`w-[64px] h-[64px] p-1 border cursor-pointer ${selectedImage === index ? 'border-[#2874f0]' : 'border-[#f0f0f0] hover:border-[#878787]'}`}
                >
          ...
          <div className="flex w-full gap-2 px-6 mt-4">
             <button 
               ...
               className="flex-1 py-[18px] bg-[#ff9f00] text-white font-bold text-[16px] rounded-sm shadow-[0_1px_2px_0_rgba(0,0,0,.2)] flex items-center justify-center gap-2"
             >
             ...
             <button 
               ...
               disabled={addingToCart || product.stock === 0}
               className="flex-1 py-[18px] bg-[#fb641b] text-white font-bold text-[16px] rounded-sm shadow-[0_1px_2px_0_rgba(0,0,0,.2)] flex items-center justify-center gap-2"
             >
        ...
        <div className="w-full md:w-[60%] p-6 md:p-8">
          ...
          {/* Breadcrumbs */}
          <div className="flex items-center text-[12px] text-[#878787] mb-2 font-medium">
          ...
          {/* Title & Brand */}
          <h1 className="text-[18px] text-[#212121] leading-relaxed mb-2">
            {product.name}
          </h1>
          ...
          {product.rating > 0 && (
            <div className="flex items-center gap-2 mb-3">
              <span className="inline-flex items-center gap-[4px] bg-[#388e3c] text-white text-[13px] font-bold px-[6px] py-[2px] rounded-sm">
              ...
              <span className="text-[14px] text-[#878787] font-medium hover:text-[#2874f0] cursor-pointer">
                {product.ratingCount?.toLocaleString()} Ratings & Reviews
              </span>
            </div>
          )}

          <div className="text-[14px] text-[#388e3c] font-medium mb-2">Extra ₹{product.discountPercent}% off</div>

          {/* Pricing */}
          <div className="flex items-baseline gap-3 mb-2">
            <span className="text-[28px] font-medium text-[#212121]">₹{price.toLocaleString()}</span>
            {originalPrice && originalPrice > price && (
              <>
                <span className="text-[16px] text-[#878787] line-through">₹{originalPrice.toLocaleString()}</span>
                <span className="text-[16px] text-[#388e3c] font-bold">{product.discountPercent}% off</span>
              </>
            )}
          </div>
          
          <div className="text-[14px] text-[#212121] mb-6 flex items-center gap-2">
             <img src="https://static-assets-web.flixcart.com/fk-p-linchpin-web/fk-cp-zion/img/fa_62673a.png" alt="FA" className="h-[21px]" />
          </div>

          {/* Available Offers */}
          <div className="mb-8">
            <div className="text-[16px] font-medium text-[#212121] mb-3">Available offers</div>
            <div className="flex items-start gap-2 mb-2">
              ...
              <div className="text-[14px] text-[#212121]">
                <strong className="font-medium">Bank Offer</strong> 5% Cashback on Flipkart Axis Bank Card <span className="text-[#2874f0] font-medium cursor-pointer">T&C</span>
              </div>
            </div>
            <div className="flex items-start gap-2 mb-2">
              <img src="https://rukminim2.flixcart.com/www/36/36/promos/06/09/2016/c22c9fc4-0555-4460-8401-bf5c28d7ba29.png?q=90" alt="tag" className="w-[18px] h-[18px] mt-0.5" />
              <div className="text-[14px] text-[#212121]">
                <strong className="font-medium">Special Price</strong> Get extra {product.discountPercent}% off (price inclusive of cashback/coupon) <span className="text-[#2874f0] font-medium cursor-pointer">T&C</span>
              </div>
            </div>
          </div>
          ...
          <div className="border border-[#f0f0f0] rounded-sm p-6 mt-8">
            <h2 className="text-[18px] font-medium text-[#212121] mb-4">Specifications</h2>
            <div className="flex flex-col">
              <div className="text-[16px] font-medium text-[#212121] border-b border-[#f0f0f0] pb-3 mb-3">General</div>
              <div className="grid grid-cols-1 md:grid-cols-[30%_1fr] gap-4">
                 <div className="text-[14px] text-[#878787]">Brand</div>
                 <div className="text-[14px] text-[#212121]">{product.brand || 'Generic'}</div>
                 
                 <div className="text-[14px] text-[#878787]">Stock Status</div>
                 <div className="text-[14px] text-[#212121]">{product.stock > 0 ? `In Stock (${product.stock})` : 'Out of Stock'}</div>

                 {product.specifications && Object.entries(product.specifications).map(([key, value]) => (
                   <React.Fragment key={key}>
                     <div className="text-[14px] text-[#878787] capitalize">{key}</div>
                     <div className="text-[14px] text-[#212121]">{value}</div>
                   </React.Fragment>
                 ))}
              </div>
            </div>
          ...
          {product.description && (
             <div className="mt-8 border border-[#f0f0f0] rounded-sm p-6">
                <h2 className="text-[18px] font-medium text-[#212121] mb-4">Product Description</h2>
                <div className="text-[14px] text-[#212121] leading-[1.8]">
                  {product.description}
                </div>
             </div>
          )}
