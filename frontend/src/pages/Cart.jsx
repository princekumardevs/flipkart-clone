...
function Cart() {
  ...
  if (loading) {
     return <div className="min-h-screen bg-[#f1f3f6] pt-4 p-2"><div className="w-full h-[500px] bg-white animate-pulse"></div></div>;
  }
    ...
    return (
      <div className="min-h-screen bg-[#f1f3f6] pt-4 px-[10px] pb-8">
        <div className="max-w-[1248px] mx-auto bg-white shadow-[0_1px_1px_0_rgba(0,0,0,.16)] rounded-sm min-h-[60vh] flex flex-col items-center justify-center py-10">
          ...
          <h2 className="text-[18px] text-[#212121] font-medium mb-3">Missing Cart items?</h2>
          <p className="text-[12px] text-[#878787] mb-6">Login to see the items you added previously</p>
          <button 
            onClick={() => navigate('/')}
            className="bg-[#fb641b] text-white px-16 py-[10px] rounded-sm font-medium text-[14px] shadow-sm hover:translate-y-[-1px] transition-transform"
          >
            Shop Now
          </button>
        </div>
        ...
        <div className="flex-1">
           <div className="bg-white rounded-sm shadow-[0_1px_2px_0_rgba(0,0,0,.2)] mb-4">
              <div className="px-6 py-4 flex items-center justify-between border-b border-[#f0f0f0]">
                <h2 className="text-[18px] font-medium text-[#212121]">Flipkart ({totalItems})</h2>
              </div>
              ...
              {cartItems.map((item) => (
                <div key={item.id} className="p-6 border-b border-[#f0f0f0] flex flex-col md:flex-row gap-6 relative">
                  ...
                  <div className="flex flex-col items-center gap-4 w-[112px] shrink-0">
                    ...
                    <div className="flex items-center gap-2">
                       <button 
                         ...
                         className="w-7 h-7 rounded-full border border-[#c2c2c2] bg-white flex items-center justify-center text-[18px] font-bold text-[#212121] disabled:opacity-40"
                       >-</button>
                       ...
                       <button 
                         onClick={() => updateQuantity(item.id, item.quantity + 1)}
                         className="w-7 h-7 rounded-full border border-[#c2c2c2] bg-white flex items-center justify-center text-[18px] font-bold text-[#212121]"
                       >+</button>
                    </div>
                  </div>
                  ...
                  <div className="flex-1 flex flex-col">
                    <h3 className="text-[16px] text-[#212121] font-medium hover:text-[#2874f0] cursor-pointer inline-block w-fit">{item.product.name}</h3>
                    <p className="text-[14px] text-[#878787] mt-1 mb-4">{item.product.brand || 'Generic'}</p>
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-[14px] text-[#878787] line-through">₹{item.product.originalPrice ? parseFloat(item.product.originalPrice).toLocaleString() : ''}</span>
                      <span className="text-[18px] font-medium text-[#212121]">₹{parseFloat(item.product.price).toLocaleString()}</span>
                      {discount > 0 && <span className="text-[14px] text-[#388e3c] font-medium">{item.product.discountPercent}% Off</span>}
                    </div>
                    
                    <div className="mt-8 flex items-center gap-6">
                      <button className="text-[16px] font-medium text-[#212121] hover:text-[#2874f0]">SAVE FOR LATER</button>
                      <button onClick={() => removeItem(item.id)} className="text-[16px] font-medium text-[#212121] hover:text-[#2874f0]">REMOVE</button>
                    </div>
                  </div>

                  {/* Delivery ETA */}
                  <div className="hidden md:block w-[200px] text-[14px] text-[#212121]">
                    Delivery by {new Date(Date.now() + 5 * 24 * 60 * 60 * 1000).toLocaleDateString('en-IN', { weekday: 'short', month: 'short', day: 'numeric' })} | <span className={`${deliveryCharges === 0 ? 'text-[#388e3c]' : ''}`}>{deliveryCharges === 0 ? 'Free' : `₹${deliveryCharges}`}</span>
                  </div>
                </div>
              ))}
              ...
              <div className="p-4 flex justify-end shadow-[0_-2px_10px_0_rgba(0,0,0,.1)] bg-white sticky bottom-0 z-10 w-full">
                <button 
                  ...
                  className="bg-[#fb641b] text-white px-[40px] py-[16px] rounded-sm font-medium text-[16px] shadow-sm uppercase tracking-wide"
                >
                  Place Order
                </button>
              </div>
